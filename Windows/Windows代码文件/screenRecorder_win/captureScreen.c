...
int open_device(AVFormatContext **ifmt_ctx) {
    ...
    // 指定分辨率
    if (video_size != NULL) {
        av_dict_set(&options, "videos_size", video_size, 0);
    }
...
int open_output(AVFormatContext **ofmt_ctx, AVFormatContext *ifmt_ctx, AVCodecContext *enCodecCtx, char *outUrl,
                int video_index) {
    // 用来展示函数调用结果的ret和返回错误结果的error
    char errors[1024] = {0,};
    int ret = 0;
...
void encode(AVCodecContext *enc_ctx,
    ...
    while (ret >= 0) {
        ...
        //Write PTS
        AVRational time_base = ofmt_ctx->streams[video_index]->time_base;//{ 1, 1000 };
        ...
        new_packet->duration = av_rescale_q(calc_duration, time_base_q,
...
void record_screen(AVFormatContext *ifmt_ctx,
    ...
    // 初始化SDL, 并返回渲染器和纹理
    ret = init_SDL(screen_width, screen_height, &renderer, &texture, &rect);
    if (ret < 0) {
        goto __ERROR;
    }
        ...
        if (event.type == SFM_REFRESH_EVENT) {
            ...
            if (av_read_frame(ifmt_ctx, packet) >= 0) {
                if (packet->stream_index == video_index) {
                    ...
                    if (ret < 0) {
                        av_strerror(ret, errors, 1024);
                        printf("Failed to send a packet for decoding,[%d]%s\n", ret, errors);
                        break;
                    }
                    while (ret >= 0) {
                        ...
                        if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) {
                            ...
                            av_strerror(ret, errors, 1024);
                            printf("Failed to receive a frame for decoding,[%d]%s\n", ret, errors);
                            goto __ERROR;
                        }
                        if (ret >= 0) {

                            sws_scale(img_convert_ctx, (const unsigned char *const *) frame->data,
                                      frame->linesize, 0, deCodecCtx->height,
                                      data, linesize);
                            new_frame->linesize[0] = linesize[0];
                            new_frame->linesize[1] = linesize[1];
                            new_frame->linesize[2] = linesize[2];

                            new_frame->data[0] = data[0];
                            new_frame->data[1] = data[1];
                            new_frame->data[2] = data[2];

                            // frame的pts要递增
                            new_frame->pts = base++;

                            // 完成编码和推送的功能
                            encode(enCodecCtx, new_frame, new_packet, ofmt_ctx, start_time, video_index);
            ...
            // 防止丢帧
            encode(enCodecCtx, NULL, new_packet, ofmt_ctx, start_time, video_index);
            thread_exit = 1;
